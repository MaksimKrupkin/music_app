...
function App() {
  const [isAddTrackVisible, setIsAddTrackVisible] = useState(false);

  const toggleAddTrack = () => {
    setIsAddTrackVisible(!isAddTrackVisible);
  };
    ...
    <BrowserRouter>
      <Provider store={store}>
        <Routes>
          ...
          <Route
            path="/artistProfile"
            element={<ArtistProfilePage toggleAddTrack={toggleAddTrack} />}
          />
        </Routes>
        {isAddTrackVisible && <AddTrack />}
      </Provider>
    </BrowserRouter>
  );
}
